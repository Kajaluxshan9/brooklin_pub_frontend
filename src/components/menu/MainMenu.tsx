...
export default function MainMenu() {
  ...
  const pageSize = 4;
  const [focusedItem, setFocusedItem] = useState<(typeof menuData)[number]["menuItems"][number] | null>(null);
  ...
  const menuData = [
    {
      ...
      menuItems: [
        { name: "Classic Burger", desc: "Beef, lettuce, cheese", price: "12€" , image:"https://geniusbees.s3.eu-north-1.amazonaws.com/activity-template-images/interpret-bar-graphs/clip-art-christmas.png"},
        { name: "Cheese Deluxe", desc: "Double cheese & bacon", price: "14€", image:"https://geniusbees.s3.eu-north-1.amazonaws.com/activity-template-images/interpret-bar-graphs/clip-art-christmas.png"  },
        { name: "Classic Burger", desc: "Beef, lettuce, cheese", price: "12€" , image:"https://geniusbees.s3.eu-north-1.amazonaws.com/activity-template-images/interpret-bar-graphs/clip-art-christmas.png"},
        { name: "Cheese Deluxe", desc: "Double cheese & bacon", price: "14€", image:"https://geniusbees.s3.eu-north-1.amazonaws.com/activity-template-images/interpret-bar-graphs/clip-art-christmas.png"  },
        { name: "Classic Burger", desc: "Beef, lettuce, cheese", price: "12€" , image:"https://geniusbees.s3.eu-north-1.amazonaws.com/activity-template-images/interpret-bar-graphs/clip-art-christmas.png"},
        { name: "Cheese Deluxe", desc: "Double cheese & bacon", price: "14€", image:"https://geniusbees.s3.eu-north-1.amazonaws.com/activity-template-images/interpret-bar-graphs/clip-art-christmas.png"  },
  ...
  useEffect(() => {
    const generateGridPoints = () => {
      ...
      // Fixed 3 columns per row requirement (if fewer items than 3, use numPoints)
      const isMobileLocal = sw < 768;
      // user requested each row has 3 columns — enforce that here
      const desiredCols = 3;
      ...
      const availableHeight = Math.max(300, containerHeight - padding * 2);
      const rowGap = Math.floor(availableHeight / (rows + 1));

      // size is capped but scales with cell dimensions and number of items
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        const finalY = clamp(cy, padding + size / 2, containerHeight - padding - size / 2);
      ...
      setGridConfig({ cols, rows, cellSize: Math.floor(Math.max(40, Math.min(cellWidth * 0.78, maxAllowed))), gap, containerWidth });
    };
    ...
    >
      ...
      >
        {menuData.map((item, idx) => {
          ...
          const availableHeight = containerHeight - padding * 2 - (gridConfig.gap * (gridConfig.rows - 1));
        ...
        >
              <DialogContent sx={{ p: 2, display: 'flex', justifyContent: 'center', background: 'transparent' }}>
                <Box sx={{ position: 'relative', width: 'min(920px, 96vw)', bgcolor: 'background.paper', borderRadius: 2, boxShadow: 6, p: 2 }}>
                  ...
                  {/* Grid: show up to 4 images horizontally per page */}
                  <AnimatePresence mode="wait">
                  <Box key={pageIndex} sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2,1fr)', sm: 'repeat(4,1fr)' }, gap: 2 }}>
                    {(
    ...
    >
      ...
      >
        ›
                  ...
                  {/* Main image fixed at bottom of panel (static across pages) */}
                  <motion.div layoutId="mainImage" style={{ marginTop: 16 }}>
                    <Box
                      component="img"
                      src={selectedItem?.mainImage}
                      alt={selectedItem?.name + " main"}
                      loading="lazy"
                      sx={{
                        width: { xs: '70%', sm: '50%', md: '30%', lg: '20%' },
                        height: 'auto',
                        maxHeight: { xs: 240, sm: 320, md: 420 },
                        objectFit: 'cover',
                        borderRadius: 1,
                        justifyContent: 'center',
                        display: 'block',
